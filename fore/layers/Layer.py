...
class Transformer_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # ------------------ padding 对齐 ------------------
        t_new = new_x.size(1)
        t_intra = intra_out_concat.size(1)
        t_inter = inter_out.size(1)

        max_t = max(t_new, t_intra, t_inter)

        def pad_to(tensor, target_len):
            cur_len = tensor.size(1)
            if cur_len < target_len:
                pad_len = target_len - cur_len
                tensor = F.pad(tensor, (0, 0, 0, 0, 0, pad_len))
            return tensor

        new_x = pad_to(new_x, max_t)
        intra_out_concat = pad_to(intra_out_concat, max_t)
        inter_out = pad_to(inter_out, max_t)

        out = new_x + intra_out_concat + inter_out  

        # ------------------ BatchNorm & FFN ------------------
        if self.batch_norm:
            out = self.norm_attn(out.reshape(b * nvar, max_t, self.d_model))
            out = out.reshape(b, nvar, max_t, self.d_model).permute(0, 2, 1, 3)  

        out = self.dropout(out)
        out = self.ff(out) + out

        if self.batch_norm:
            out = self.norm_ffn(out.reshape(b * nvar, max_t, self.d_model))
            out = out.reshape(b, nvar, max_t, self.d_model).permute(0, 2, 1, 3)
